...
;; registers under the control stack pointer.
  ...
  (flet ((reg (offset sc)
    ...
    (let* ((nl-registers (loop for i to 9
      ...
      (macrolet  ((map-pairs (op base start offsets
        ...
        (inst add csp-tn csp-tn (+ descriptor-framesize 16))
        (map-pairs stp csp-tn (- (+ descriptor-framesize 16)) descriptor-registers)
        (inst stp cfp-tn csp-tn (@ thread-tn (* thread-control-frame-pointer-slot n-word-bytes)))
        (inst stp cfp-tn lr-tn (@ csp-tn -16))
        ...
        (map-pairs ldp nsp-tn 480 float-registers :post-index 512 :delta -32)
        (map-pairs ldp csp-tn -32 descriptor-registers :delta -16)
        (inst ldr lr-tn (@ csp-tn -8))

        ;; Deallocate the frame.
        (inst sub csp-tn csp-tn (+ 16 descriptor-framesize))
        (inst str zr-tn (@ thread-tn (* thread-control-stack-pointer-slot n-word-bytes)))
