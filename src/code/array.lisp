...
(define-load-time-global %%blt-copier-for-widetag%% (make-array 32 :initial-element nil))
...
(defun !blt-copiers-cold-init ()
  (setf %%blt-copier-for-widetag%% (make-array 32 :initial-element nil))
  (macrolet ((init ()
               `(progn
                  ,@(loop for saetp across *specialized-array-element-type-properties*
                          when (and (not (member (saetp-specifier saetp) '(t nil)))
                                    (<= (saetp-n-bits saetp) n-word-bits))
                            collect `(setf (svref %%blt-copier-for-widetag%% ,(ash (- (saetp-typecode saetp) 128) -2))
                                           #',(intern (format nil "UB~D-BASH-COPY"
                                                              (saetp-n-bits saetp))
                                                      "SB-KERNEL"))))))
    (init)))

(defmacro blt-copier-for-widetag (x)
  `(aref %%blt-copier-for-widetag%% (ash (- ,x 128) -2)))
