...
(with-test (:name :package-compile/load-consistency.spec.3.2.4.4.symbol.1a)
  (with-scratch-file (fasl8 "fasl")
    (compile-file "package-test-8.lisp" :output-file fasl8)
    (assert-error (load fasl8)
                  sb-int:package-compile-load-consistency-error)
    (delete-package "FOO")))

(with-test (:name :package-compile/load-consistency.spec.3.2.4.4.symbol.1b)
  (with-scratch-file (fasl1 "fasl")
    (let ((package (make-package "COMPILE-PACKAGE" :use '("CL"))))
      (let ((*package* package))
        (compile-file "package-test-1.lisp" :output-file fasl1)
        (delete-package package)))
    (let ((package (make-package "LOAD-PACKAGE" :use '("CL"))))
      (let ((*package* package))
        (assert-error (load fasl1)
                      sb-int:package-compile-load-consistency-error)
        (delete-package package)))))
